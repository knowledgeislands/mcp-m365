...
4. **All Zod schemas must be `.strict()` with bounded numerics.** This is a **known gap** — several tools currently pass plain `{...}` input objects rather than `.strict()` Zod schemas, and numeric inputs like `count` lack `min`/`max` bounds (could request 10⁶ items and exhaust Graph quota). When touching a tool, convert its `inputSchema` to a `.strict()` Zod object and bound numeric fields (e.g. `z.number().int().min(1).max(50)`).
5. **Destructive tools should expose `dry_run`.** **Known gap** — `email/delete`, `calendar/delete`, `calendar/cancel`, `onedrive/delete`, `folder/delete` currently execute immediately with no preview. Plan: add `dry_run: boolean` (default `true`) that returns the target's metadata (subject/path/size) without calling the destructive Graph endpoint. Annotate all such tools with `DESTRUCTIVE_REMOTE` from [src/utils/annotations.ts](./src/utils/annotations.ts).
6. **`outputPath` and downloaded-file paths need a safe-root boundary.** Tools that write Graph response bytes to disk (`onedrive_download`, attachment-style flows) currently accept any writable path. **Known gap** — add a configurable download root (e.g. `MCP_M365_DOWNLOAD_PATH`, default `~/Downloads`) and validate `outputPath` against it before `fs.writeFile`.
7. **Microsoft Graph paths must not interpolate user input into URL fragments unchecked.** `onedrive/download.ts` does `me/drive/root:/${normalizedPath}` after stripping leading/trailing slashes. Graph itself validates the path, but defense-in-depth: encode segments with `encodeURIComponent` and reject `:` (Graph's path/id separator) in user input.
