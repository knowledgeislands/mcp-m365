...
Run `bun run` with no args for the full script list. `bun run ki:test:smoke` boots the server over stdio and asserts the wire-level tool surface matches `EXPECTED_TOOLS` in [scripts/smoke.ts](./scripts/smoke.ts) — keep that list in sync when adding or removing tools. It also asserts the live protocol profile: modern era, negotiated `2026-07-28`, a complete `server/discover` envelope, a real `tools/call` round trip, argument rejection, and that a deliberately legacy client still sees the identical surface. Source audit deliberately does not require a local discovery literal, so this script is the only place those claims are proved.
...
- **[src/mcp-server/index.ts](./src/mcp-server/index.ts)** — the stdio MCP wrapper. Calls `loadConfig()` once, then threads `Config` into the access gate, the shared token storage (`initTokenStorage(config)`), and tool registration. Startup logging reads from `config`. The stdio boundary is the SDK's `serveStdio(createServer, { legacy: 'serve' })` on the modern 2026-07-28 profile: `createServer` is a **per-connection factory**, so the `McpServer` instance, the access gate wrapped around it, and the seven `register*Tools(...)` calls all belong to one connection. Connection-independent state (config, token storage, the Graph and triage contexts) stays at module scope above the factory — rebuilding token storage per connection would fragment the refresh path. `legacy: 'serve'` is a deliberate retention while the fleet migrates, not an oversight.
...
1. **The server holds no rule state.** Rules either arrive as a string in the call or are read at call time from a path; either way the caller's rule file is canonical and is re-read every time. Do not add server-side rule storage or caching.
2. **Every filesystem path goes through `assertWithinRoots()`.** Rules and tracking paths may be supplied per call, so the allowlist in [src/utils/paths.ts](./src/utils/paths.ts) is the only thing standing between a tool argument and arbitrary file access. It is deliberately two-layer — lexical `resolve` then `realpath` — because the knowledge base symlinks `Pillars/*` into a separate repository, and a lexical-only check is defeated by a symlink inside a root pointing out of it. Never read or write an engine path that has not been through it.
3. **Message identity is subject + sender + received timestamp, never the Graph id.** Graph reissues ids on folder moves, so an id is a cache hint that `findMessage` re-verifies before any mutation — and a `move` reissues the id again, which is why `applyOne` threads the new one into the actions that follow.
...
5. **Ordering is the whole specification.** The list is flat and first-match-wins, so the two lint checks that matter are `shadowed-rule` (a rule that can never fire) and `broad-rule-collision` (a content-agnostic rule sitting above the specific rules it will swallow — the class of bug that sent project calendar invites to the action folder). A reviewed collision is silenced with a `# lint:allow-collision` comment on the rule, not by weakening the check.
