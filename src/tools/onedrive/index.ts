...
export const onedriveTools = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: "Path to list (e.g., '/Documents', '/Photos'). Defaults to root." },
        count: { type: 'number', description: 'Number of items to retrieve (default: 25, max: 50)' }
      },
      required: []
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        count: { type: 'number', description: 'Number of results to return (default: 25, max: 50)' }
      },
      required: ['query']
    },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        itemId: { type: 'string', description: 'ID of the item to download' },
        path: { type: 'string', description: 'Path to the file (alternative to itemId)' }
      },
      required: []
    },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: "Destination path including filename (e.g., '/Documents/myfile.txt')" },
        content: { type: 'string', description: 'File content to upload' },
        conflictBehavior: { type: 'string', description: "Behavior when file exists: 'rename' (default), 'replace', or 'fail'", enum: ['rename', 'replace', 'fail'] }
      },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: "Destination path including filename (e.g., '/Documents/largefile.zip')" },
        content: { type: 'string', description: 'File content to upload' },
        conflictBehavior: { type: 'string', description: "Behavior when file exists: 'rename' (default), 'replace', or 'fail'", enum: ['rename', 'replace', 'fail'] }
      },
      required: ['path', 'content']
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        type: { type: 'string', description: "Link type: 'view' (default), 'edit', or 'embed'", enum: ['view', 'edit', 'embed'] },
        scope: { type: 'string', description: "Link scope: 'anonymous' (default) or 'organization'", enum: ['anonymous', 'organization'] }
      },
      required: []
    },
    handler: handleShare
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: "Parent folder path (e.g., '/Documents'). Defaults to root." },
