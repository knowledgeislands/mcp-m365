...
export const getFolderIdByName = async (accessToken: string, folderName: string): Promise<string | null> => {
  ...
  return resolvePathSegments(allFolders, segments, folderName)
}
...
const resolvePathSegments = (allFolders: any[], segments: string[], originalName: string): string | null => {
  ...
  if (candidates.length > 1) {
    console.error(`Ambiguous folder path "${originalName}" — ${candidates.length} folders match: ${candidates.map((f: any) => f.id).join(', ')}`)
    return null
  }
  return null
}
