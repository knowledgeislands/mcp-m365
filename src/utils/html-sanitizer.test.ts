...
describe('HTML Sanitizer Security Tests', () => {
  ...
  describe('hasHidingAttributes', () => {
    test('returns false for null/undefined input', () => {
      expect(hasHidingAttributes(null)).toBe(false)
      expect(hasHidingAttributes(undefined)).toBe(false)
    })

    test('returns true when the `hidden` attribute is present', () => {
      expect(hasHidingAttributes({ hidden: '' })).toBe(true)
    })

    test('returns true when aria-hidden is "true"', () => {
      expect(hasHidingAttributes({ 'aria-hidden': 'true' })).toBe(true)
    })

    test('ignores aria-hidden values other than "true"', () => {
      expect(hasHidingAttributes({ 'aria-hidden': 'false' })).toBe(false)
    })

    test('returns true when style contains a hiding rule', () => {
      expect(hasHidingAttributes({ style: 'display: none' })).toBe(true)
    })

    test('returns true for class names that match hiding patterns', () => {
      expect(hasHidingAttributes({ class: 'sr-only banner' })).toBe(true)
      expect(hasHidingAttributes({ class: 'visually-hidden' })).toBe(true)
      expect(hasHidingAttributes({ class: 'screen-reader-only' })).toBe(true)
      expect(hasHidingAttributes({ class: 'is-hide' })).toBe(true)
      expect(hasHidingAttributes({ class: 'invisible' })).toBe(true)
    })

    test('returns false for benign class names', () => {
      expect(hasHidingAttributes({ class: 'callout banner' })).toBe(false)
    })

    test('returns false for empty attribute objects', () => {
      expect(hasHidingAttributes({})).toBe(false)
    })
  })

  describe('Blockquote handling', () => {
    test('prefixes each line of a blockquote with "> "', () => {
    ...
    test('handles a single-line blockquote', () => {
      const result = sanitizeHtmlToText('<blockquote>quoted text</blockquote>')
      expect(result).toContain('> quoted text')
    })
  })

  describe('HTML entity decoding (numeric)', () => {
    test('decodes decimal numeric entities', () => {
      // &#65; = "A", &#8364; = "€"
      expect(sanitizeHtmlToText('<p>&#65;&#8364;</p>')).toContain('A€')
    })

    test('drops decimal entities outside the BMP range', () => {
      // 0 and 65536 are out-of-range and should be removed (return '')
      const result = sanitizeHtmlToText('<p>x&#0;y&#65536;z</p>')
    ...
    test('drops hex entities outside the BMP range', () => {
      const result = sanitizeHtmlToText('<p>x&#x0;y&#x10000;z</p>')
      expect(result).toContain('xyz')
    })
  })
  ...
  describe('wrapEmailContent', () => {
    test('omits header lines for missing metadata fields', () => {
      const wrapped = wrapEmailContent('body', {})
      expect(wrapped).not.toContain('From:')
      expect(wrapped).not.toContain('Subject:')
      expect(wrapped).not.toContain('Date:')
      expect(wrapped).toContain('body')
    })

    test('includes the Date header when supplied', () => {
      const wrapped = wrapEmailContent('body', { date: '2026-05-09 09:00' })
      expect(wrapped).toContain('Date: 2026-05-09 09:00')
    })

    test('includes all three header fields when supplied', () => {
      const wrapped = wrapEmailContent('body', { from: 'a@x.com', subject: 'Hi', date: '2026-05-09' })
      expect(wrapped).toContain('From: a@x.com')
      expect(wrapped).toContain('Subject: Hi')
      expect(wrapped).toContain('Date: 2026-05-09')
    })
  })

  describe('processHtmlEmail boundary toggle', () => {
    test('omits boundary markers when addBoundary=false', () => {
      const r = processHtmlEmail('<p>hi</p>', { addBoundary: false })
      expect(r).not.toContain('EMAIL CONTENT START')
