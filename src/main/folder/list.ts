...
/**
 * Shape of the `structuredContent` returned by `handleListFolders`, and (via the
 * same schema) the `outputSchema` declared by the `m365_email_folders_list`
 ...
 */
export const folderListResultSchema = z
  .object({
    type: z.literal('folder-list'),
    success: z.boolean(),
    includeItemCounts: z.boolean().optional(),
...
const formatFolderHierarchy = (folders: any[], includeItemCounts: boolean): string => {
  ...
  folders.forEach((folder) => {
    if (!folder.isTopLevel && folder.parentFolderId) {
      ...
      /* v8 ignore next 4 — a non-top-level folder's parent is always in the set, so it is always found */
      if (parent) {
  ...
  function formatSubtree(folderId: string, level: number = 0): string {
    ...
    /* v8 ignore next — folderId always comes from folderMap keys, so the lookup never misses */
    if (!folder) return ''
